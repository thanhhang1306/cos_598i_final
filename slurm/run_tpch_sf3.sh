...
#SBATCH --output=results/tpch_sf3_%j.out
#SBATCH --error=results/tpch_sf3_%j.err
...
DATA="$PROJECT/data/tpch/sf3/"

[[ -x "$BIN" ]] || { echo "missing binary: $BIN (build it first)"; exit 1; }
[[ -d "$DATA" ]] || { echo "missing data dir: $DATA (generate SF=3 data via 3rdparty/tpch-dbgen)"; exit 1; }
